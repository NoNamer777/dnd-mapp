...
describe('DmaAuthenticationService', () => {
    ...
    async function setupTestEnvironment(params: { authenticated?: boolean; withToken?: string } = {}) {
        ...
        cookieSpy.and.returnValue(
            params.withToken ? params.withToken : !params.authenticated ? '' : identityTokenCookie
        );
    ...
    it('should process an sign-out request', async () => {
        const { authenticationService, testingController, cookieSpy } = await setupTestEnvironment({
            authenticated: true,
        });

        expect(authenticationService.authenticatedUser$.value).not.toBeNull();

        const signOut = lastValueFrom(authenticationService.signOut());

        cookieSpy.and.returnValue('');
        testingController.expectOne(environment.baseBackEndURL + '/authentication/sign-out').flush(null);
    ...
    describe('setAuthentication', () => {
        it('should initialize an authenticated User when User data is returned based on the presented cookies', async () => {
            const { authenticationService } = await setupTestEnvironment({ authenticated: true });
        ...
        it('should not initialize with an invalid token in the cookie', async () => {
            spyOn(console, 'error');

            const { authenticationService } = await setupTestEnvironment({ withToken: 'identity-token=token' });
    ...
    describe('login', () => {
        it('should be processing login requests without throwing errors.', fakeAsync(async () => {
            ...
            const challengeRequest = testingController.expectOne(
                environment.baseBackEndURL + '/authentication/challenge'
            );

            expect(challengeRequest.request.body).toEqual(
                jasmine.objectContaining({ codeChallenge: jasmine.any(String) })
            );
            ...
            const authorizeRequest = testingController.expectOne(
                environment.baseBackEndURL + '/authentication/authorize'
            );
            authorizeRequest.flush({
                state: authorizeRequest.request.body.state,
